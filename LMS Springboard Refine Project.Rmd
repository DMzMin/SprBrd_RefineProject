...
In this exercise the data will be cleaned up so that company names are consistent for each company, the 'product.code' and 'product.number' are separated and a new variable 'product_categpory' based upon the 'product.code' will be added.  'Address', 'city' and 'country' will be combined into a new column 'full_address' and dummy numerical variables will be made for all company and product categories.  The clean data set will be saved and written to 'refine_clean.csv'.

Please note that the column 'product.code...number' is a changed name from the original 'refine.xlsx' where it is named 'product code / product number'.  This doe not always convert to 'product.code...number' in all possible conversion from .xlsx to .csv file types.
...
Finally read and write clead data set to 'refine_clean.csv'
